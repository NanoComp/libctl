  ...
  switch (obj.which_subclass) {
    case CYLINDER:
      printf("          cylinder with height %g, axis (%g, %g, %g)\n",
             obj.subclass.cylinder_data->height, obj.subclass.cylinder_data->axis.x,
             obj.subclass.cylinder_data->axis.y, obj.subclass.cylinder_data->axis.z);
      break;
    case SPHERE:
    ...
    case BLOCK:
      printf("          block with size (%g,%g,%g)\n", obj.subclass.block_data->size.x,
             obj.subclass.block_data->size.y, obj.subclass.block_data->size.z);
      ...
      break;
    case GEOMETRIC_OBJECT_SELF: printf("          generic geometric object\n"); break;
...
void run_program(void) {
  ...
  printf("Epsilon of (%g, %g) is %g (tree) or %g (non-tree)\n", p.x, p.y,
         material_of_point_in_tree(p, t).epsilon, material_of_point(p).epsilon);
  ...
  if (num_write_output_vars > 1) destroy_output_vars(); /* we are responsible for calling this */

  printf("dummy = (%g+%gi, %g+%gi, %g+%gi)\n", dummy.x.re, dummy.x.im, dummy.y.re, dummy.y.im,
         dummy.z.re, dummy.z.im);
  dummy = make_cvector3(vector3_scale(2, cvector3_re(dummy)), vector3_scale(3, cvector3_im(dummy)));
...
number function_func(function func, number arg) {
  return ctl_convert_number_to_c(gh_call1(func, ctl_convert_number_to_scm(arg)));
}
